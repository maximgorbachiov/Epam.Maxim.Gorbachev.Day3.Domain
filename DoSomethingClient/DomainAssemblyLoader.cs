...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result Load(string assemblyString, Input data)
        {
            ...
            var doSomethingType = types
                .Where(type => (type.GetCustomAttribute(typeof(DoSomethingAttribute)) != null) && (type.GetInterface("IDoSomething") != null))
                .First();
            // TODO: Find first type that has DoSomething attribute and implements IDoSomething.
            // TODO: Create an instance of this type.

            IDoSomething doSomethingService = (IDoSomething)(Activator.CreateInstance(doSomethingType)); // TODO Save instance to variable.
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            var doSomethingType = types
                .Where(type => (type.GetCustomAttribute(typeof(DoSomethingAttribute)) != null) && (type.GetInterface("IDoSomething") == null))
                .First();
            var doSmthObj = Activator.CreateInstance(doSomethingType);

            // TODO: Find first type that has DoSomething attribute and don't implement IDoSomething.
            MethodInfo mi = doSomethingType.GetMethod("DoSomething");
            Result result = (Result)(mi.Invoke(doSmthObj, new object[] { data }));
